...
실행: streamlit run frontend/app_housing.py --server.port 8501
"""
...
with col_input:
    ...
    c1, c2 = st.columns(2)
    with c1:
        med_inc = st.number_input("중위 소득 (MedInc)", min_value=0.1, max_value=20.0, value=3.5, step=0.1)
    with c2:
    ...
    c1, c2 = st.columns(2)
    with c1:
        latitude = st.number_input("위도 (Latitude)", min_value=32.0, max_value=42.0, value=37.5, step=0.1)
    with c2:
